...
internal data class OmniFlowRuntimeManifest(
    val version: String,
    val protocol: String,
...
internal fun parseOmniFlowRuntimeManifest(input: InputStream): OmniFlowRuntimeManifest {
    ...
    val bundleSha256 = required("bundle.sha256").lowercase()
    require(bundleSha256.matches(Regex("[a-f0-9]{64}"))) { "omniflow_runtime_sha256_invalid" }
    fun sourceSha256(name: String): String = required(name).lowercase().also { value ->
        require(value.matches(Regex("[a-f0-9]{64}"))) { "omniflow_runtime_source_sha256_invalid:$name" }
    }
    ...
    return OmniFlowRuntimeManifest(
        ...
        platform = required("runtime.platform"),
...
internal fun extractOmniFlowRuntimeBundle(bundle: InputStream, targetDirectory: File) {
    val targetRoot = targetDirectory.canonicalFile
    targetRoot.mkdirs()
    ZipInputStream(bundle.buffered()).use { zip ->
        while (true) {
            val entry = zip.nextEntry ?: break
            val target = File(targetRoot, entry.name).canonicalFile
            require(
                target.path == targetRoot.path ||
                    target.path.startsWith(targetRoot.path + File.separator)
            ) { "omniflow_runtime_zip_entry_invalid:${entry.name}" }
            if (entry.isDirectory) {
                target.mkdirs()
            } else {
                target.parentFile?.mkdirs()
                target.outputStream().buffered().use(zip::copyTo)
            }
            zip.closeEntry()
        }
    }
