 ...
 */
class OmniFlowFunctionStore(private val workspaceRoot: File) {
    ...
    fun register(spec: Map<String, Any?>): Map<String, Any?> {
        val functionId = OmniFlowFunctionSchema.functionId(spec).takeIf { it.isNotEmpty() }
            ?: return mapOf("success" to false, "errorMessage" to "function_id required")
        val existing = get(functionId)
        val sourceRunIds = (existing?.let(OmniFlowFunctionSchema::sourceRunIds).orEmpty() +
            OmniFlowFunctionSchema.sourceRunIds(spec)).distinct()
        val now = System.currentTimeMillis().toString()
        val existingRegistry = OmniFlowFunctionJson.mapArg(existing?.get("_oob_registry"))
        val incomingRegistry = OmniFlowFunctionJson.mapArg(spec["_oob_registry"])
        val storedSpec = linkedMapOf<String, Any?>().apply {
            ...
            if (sourceRunIds.isNotEmpty()) {
                put(
                    "metadata",
                    linkedMapOf<String, Any?>().apply {
                        putAll(OmniFlowFunctionJson.mapArg(existing?.get("metadata")))
                        putAll(OmniFlowFunctionJson.mapArg(spec["metadata"]))
    ...
    fun functionIds(limit: Int = 500): List<String> =
        list(limit).mapNotNull { spec ->
            OmniFlowFunctionSchema.functionId(spec).takeIf { it.isNotEmpty() }
        }
    ...
    fun recordRun(
        ...
        val now = System.currentTimeMillis().toString()
        val existingRegistry = OmniFlowFunctionJson.mapArg(existing["_oob_registry"])
        val existingStats = OmniFlowFunctionJson.mapArg(existingRegistry["run_stats"])
    ...
    fun distillFromRun(record: InternalRunLogRecord): Map<String, Any?> {
        ...
        return mapOf(
            ...
            "step_count" to OmniFlowFunctionSchema.materializedSteps(storedSpec).size,
    ...
    internal fun compileRunLogFunctionSpec(record: InternalRunLogRecord): Map<String, Any?>? {
        return OmniFlowFunctionCompiler.compile(record)
    }
...
private class RecallIndex(private val workspaceRoot: File) {
    ...
    fun upsert(spec: Map<String, Any?>) {
        val functionId = OmniFlowFunctionSchema.functionId(spec).takeIf { it.isNotEmpty() } ?: return
    ...
    fun search(goal: String, limit: Int): List<OmniFlowFunctionStore.RecallHit> {
                    ...
                    buildList {
                        ...
                        while (cursor.moveToNext()) {
                            ...
                            if (functionId.isNotEmpty() && score > 0.0) {
                                add(OmniFlowFunctionStore.RecallHit(functionId = functionId, score = score))
                            }
                        }
                    }
                        .sortedWith(compareByDescending<OmniFlowFunctionStore.RecallHit> { it.score }.thenBy { it.functionId })
    ...
    private fun searchableText(spec: Map<String, Any?>): String {
        val source = OmniFlowFunctionJson.mapArg(spec["source"])
        val metadata = OmniFlowFunctionJson.mapArg(spec["metadata"])
        val agentReuse = OmniFlowFunctionJson.mapArg(spec["agent_reuse"])
            .ifEmpty { OmniFlowFunctionJson.mapArg(metadata["agent_reuse"]) }
        val constraints = OmniFlowFunctionJson.mapArg(spec["constraints"])
        val schema = OmniFlowFunctionSchema.inputSchema(spec)
        val parameters = OmniFlowFunctionJson.mapArg(schema["properties"]).flatMap { (name, raw) ->
            val property = OmniFlowFunctionJson.mapArg(raw)
            listOf(name, property["title"], property["description"])
        }
        val steps = OmniFlowFunctionSchema.stepSummaries(spec).flatMap { step ->
            listOf(step["title"], step["tool"])
        }
        val text = buildList {
            add(OmniFlowFunctionSchema.functionId(spec))
            add(spec["name"])
    ...
    private companion object {
        private const val TAG = "OmniFlowFunctionStore.RecallIndex"
