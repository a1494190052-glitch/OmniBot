...
/**
 * Normalizes raw step Maps into the canonical Function step format.
 * Used by FunctionService simple registration.
 */
internal object FunctionStepNormalizer {
    ...
    fun normalizeSimpleRegisteredStep(
        ...
        val normalizedTool = OobActionSchema.normalizeToolName(rawTool)
        ...
        when {
            ...
            FunctionSchema.isFunctionCallTool(normalizedTool) ||
                    firstNonBlank(stepArgs["function_id"]).isNotBlank() -> {
                step["kind"] = "function"
                step["model_free"] = true
                step["scriptable"] = true
                step["tool"] = OobActionSchema.TOOL_CALL_TOOL
    ...
    fun executionCapabilities(steps: List<Map<String, Any?>>): Map<String, Any?> = linkedMapOf(
        "function_step_count" to steps.count { step ->
            FunctionSchema.isFunctionStepTool(firstNonBlank(step["tool"]))
        },
    )
