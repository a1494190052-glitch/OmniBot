...
internal suspend fun ReplayHelper.evaluateWorkspaceRule(
    ...
    OmniflowCheckerRule.COND_XPATH_EXISTS,
    OmniflowCheckerRule.COND_TARGET_COVERED_BY_XPATH -> checkerXPathRule(deviceOperator, rule, state, replayAction)
    OmniflowCheckerRule.COND_PACKAGE_MISMATCH -> checkerPackageMismatch(deviceOperator, rule, state, replayAction)
    OmniflowCheckerRule.COND_KEYBOARD_OBSCURING -> checkerKeyboardObscuring(deviceOperator, rule, state, replayAction)
    else -> null
}
...
private suspend fun ReplayHelper.checkerXPathRule(
    ...
    val conditionXPath = when (rule.condition) {
        OmniflowCheckerRule.COND_TARGET_COVERED_BY_XPATH ->
    ...
    if (rule.condition == OmniflowCheckerRule.COND_TARGET_COVERED_BY_XPATH &&
    ...
    return when (rule.action) {
        OmniflowCheckerRule.ACTION_CLICK -> {
        ...
        OmniflowCheckerRule.ACTION_WAIT -> {
            ...
            linkedMapOf(
                ...
                "controller" to rule.id,
                "condition" to rule.condition,
                "action" to OmniflowCheckerRule.ACTION_WAIT,
                "delay_ms" to delayMs,
            )
        }
        else -> null
