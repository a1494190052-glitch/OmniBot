 ...
 * owned by OmniFlowFunctionRun.
 */
class OmniFlowFunctionService(
    private val context: Context,
    private val deviceOperator: DeviceOperator = AndroidDeviceOperator(null, context),
    private val workspaceFunctionStore: OmniFlowFunctionStore = OmniFlowFunctionStore(
    ...
    suspend fun executeTool(name: String?, args: Map<String, Any?>?): Map<String, Any?> {
        return when (name) {
            ...
            OmniFlowFunctionApi.FUNCTION_LIST -> listFunctions(args)
            OmniFlowFunctionApi.FUNCTION_GET -> getFunction(args)
            OmniFlowFunctionApi.FUNCTION_REGISTER -> registerFunction(args)
            OmniFlowFunctionApi.FUNCTION_UPDATE -> updateFunction(args)
            OmniFlowFunctionApi.FUNCTION_DELETE -> deleteFunction(args)
            OmniFlowFunctionApi.FUNCTION_CLEAR -> clearFunctions(args)
            OmniFlowFunctionApi.RUN_LOG_LIST -> listRunLogs(args)
            OmniFlowFunctionApi.RUN_LOG_GET -> getRunLog(args)
            OmniFlowFunctionApi.RUN_LOG_CONVERT -> convertRunLog(args)
    ...
    fun recall(args: Map<String, Any?>?): Map<String, Any?> {
        ...
        return linkedMapOf<String, Any?>(
            ...
            "source" to "omniflow_function_recall",
    ...
    fun getFunction(args: Map<String, Any?>?): Map<String, Any?> {
        ...
        return linkedMapOf<String, Any?>().apply {
            ...
            put("function_id", firstNonBlank(OmniFlowFunctionSchema.functionId(spec), functionId))
    ...
    fun registerFunction(args: Map<String, Any?>?): Map<String, Any?> {
        ...
        val registeredId = firstNonBlank(
            ...
            OmniFlowFunctionSchema.functionId(functionSpec),
        )
    ...
    private fun buildFunctionSpecForRegistration(request: Map<String, Any?>): Map<String, Any?> {
        ...
        val normalizedSteps = rawSteps.mapIndexed { index, raw ->
            cn.com.omnimind.bot.omniflow.function.OmniFlowFunctionStepNormalizer.normalizeSimpleRegisteredStep(
                raw = raw,
                index = index,
                inheritedSourceContext = if (index == 0) sourceContext else emptyMap(),
            )
        }
        val capabilities = cn.com.omnimind.bot.omniflow.function.OmniFlowFunctionStepNormalizer.executionCapabilities(normalizedSteps)
    ...
    private fun saveFunctionSpec(functionSpec: Map<String, Any?>): Map<String, Any?> {
        val rawSpec = OmniFlowFunctionParameterBindingNormalizer.normalize(
            OmniFlowFunctionJson.sanitizeMap(functionSpec)
        )
        ...
        return linkedMapOf(
            ...
            "runner" to "oob_agent_reusable_function",
            "parameter_binding_normalization" to OmniFlowFunctionJson.mapArg(spec["metadata"])
    ...
    private fun listFunctionsPage(
        ...
        return linkedMapOf(
            ...
            "source" to "omniflow_function_service"
        )
    }
    ...
    private fun getFunctionSpec(functionId: String): Map<String, Any?>? {
        ...
        return workspaceSpec?.let { withSourceRunSummary(OmniFlowFunctionJson.sanitizeMap(it)) }
    }
    ...
    private fun summaryMap(spec: Map<String, Any?>): Map<String, Any?> {
        ...
        return linkedMapOf(
            ...
            "card_count" to (
                OmniFlowFunctionJson.intArg(source?.get("card_count"), defaultValue = 0)
                    .takeIf { it > 0 }
                    ?: OmniFlowFunctionJson.intArg(source?.get("replayable_card_count"), defaultValue = 0)
                    .takeIf { it > 0 }
                    ?: steps.size
            ),
            "omniflow_step_count" to execution?.get("omniflow_step_count"),
            "parameter_names" to OmniFlowFunctionSchema.parameterNames(spec),
            "step_summaries" to OmniFlowFunctionSchema.stepSummaries(spec),
            "function_kind" to functionKind(spec),
            "asset_state" to assetState(spec),
            ...
            "run_stats" to OmniFlowFunctionJson.sanitizeValue(
                runStats ?: emptyMap<Any?, Any?>()
            ),
            "last_run" to OmniFlowFunctionJson.sanitizeValue(
    ...
    private fun listSpecsPage(
        ...
        val visibleSpecs = workspaceFunctionStore.list(scanLimit)
            .map(OmniFlowFunctionJson::sanitizeMap)
    ...
    private fun convertStoredRunLog(
        ...
        val compiled = OmniFlowFunctionCompiler.compile(record)
            ?: return errorPayload(
        ...
        return registration.apply {
            ...
            put("source", "omniflow_function_service")
            putAll(conversionDiagnostics(record, spec, runStatusWarnings))
        }
    }
    ...
    private fun recallCandidateMap(
        spec: Map<String, Any?>,
        hit: OmniFlowFunctionStore.RecallHit,
        currentPackage: String,
    ): Map<String, Any?> {
        val functionId = OmniFlowFunctionSchema.functionId(spec)
        ...
        return linkedMapOf<String, Any?>(
            ...
            "name" to spec["name"],
            "inputSchema" to OmniFlowFunctionSchema.inputSchema(spec),
            "score" to hit.score,
            ...
            "step_count" to OmniFlowFunctionSchema.materializedSteps(spec).size,
            "step_summaries" to OmniFlowFunctionSchema.stepSummaries(spec),
    ...
    private fun functionProfile(spec: Map<String, Any?>): Map<String, Any?> {
        ...
        return linkedMapOf<String, Any?>(
            "purpose" to firstNonBlank(
                spec["description"],
                spec["name"],
                OmniFlowFunctionSchema.functionId(spec),
            ),
    ...
    private fun packageScopes(spec: Map<String, Any?>): Set<String> {
        ...
        return buildList {
            ...
            OmniFlowFunctionSchema.materializedSteps(spec).forEach { step ->
    ...
    private fun applyFunctionUpdateRequest(args: Map<String, Any?>?): Map<String, Any?> {
        ...
        val updated = OmniFlowFunctionJson.mutableJsonMap(original)
    ...
    private fun buildRunLogAnalysisContext(
        ...
        val steps = OmniFlowFunctionSchema.materializedSteps(functionSpec)
        return linkedMapOf(
    ...
    private fun buildFunctionReviewPrompt(context: Map<String, Any?>): String {
        val contextJson = JSONObject(OmniFlowFunctionJson.sanitizeMap(context)).toString(2)
        ...
        return """
            Analyze this OmniFlow Function with the provided RunLog evidence, then return exactly one JSON object with top-level keys "analysis" and "patch".
    ...
    private fun applyRunLogEvidenceAnalysis(
        ...
        val metadata = OmniFlowFunctionJson.mutableJsonMap(mapArg(spec["metadata"]))
        val existing = OmniFlowFunctionJson.mutableJsonMap(mapArg(metadata["oob_function_evidence"]))
        ...
        val evidence = linkedMapOf<String, Any?>().apply {
            ...
            put("latest_analysis", OmniFlowFunctionJson.mutableJsonValue(analysis))
    ...
    private fun appendUpdateAudit(
        ...
        metadata["oob_function_update"] = linkedMapOf(
            ...
            "tool" to OmniFlowFunctionApi.FUNCTION_UPDATE,
            "mode" to mode,
        ...
        if (mode == "enhance" || metadata["oob_enhancement"] != null) {
            metadata["oob_enhancement"] = linkedMapOf(
                ...
                "source" to OmniFlowFunctionApi.FUNCTION_UPDATE,
    ...
    private fun applyStepLabelPatches(
        ...
        val execution = OmniFlowFunctionJson.mutableJsonMap(mapArg(spec["execution"]))
        val steps = OmniFlowFunctionJson.mutableJsonList(listArg(execution["steps"]))
        stepPatches.forEach { sp ->
            ...
            val step = OmniFlowFunctionJson.mutableJsonMap(mapArg(steps[stepIndex]))
            ...
            if (cleanup.isNotEmpty()) {
                val metadata = OmniFlowFunctionJson.mutableJsonMap(mapArg(step["metadata"]))
        ...
        metadataPatch.forEach { (key, value) ->
            when (key) {
                "checker_rules", "checkerRules" -> changes += applyCheckerRulesPatch(metadata, value)
                    ...
                    metadata[key] = OmniFlowFunctionJson.mutableJsonValue(value)
    ...
    private fun sanitizeCheckerRule(raw: Map<String, Any?>): Map<String, Any?>? {
        ...
        if (!OmniflowCheckerRule.isSupportedPair(condition, action)) return null
    ...
    private fun ingestInlineRunLog(
        ...
        val runStatusWarnings = runStatusWarnings(record)
        val spec = OmniFlowFunctionCompiler.compile(record)
            ?: return errorPayload(
                code = "RUN_LOG_NO_REPLAYABLE_STEPS",
                message = "RunLog has no replayable steps"
    ...
    private fun functionAgentPrompt(spec: Map<String, Any?>): String {
        val functionId = firstNonBlank(OmniFlowFunctionSchema.functionId(spec), spec["function_id"])
    ...
    private fun stepSummaries(spec: Map<String, Any?>): List<Map<String, Any?>> {
        return OmniFlowFunctionSchema.stepSummaries(spec)
    }

    private fun materializedSteps(spec: Map<String, Any?>): List<Map<String, Any?>> {
        return OmniFlowFunctionSchema.materializedSteps(spec)
    }
    ...
    private fun analysisFromAgentUpdateJson(json: Map<String, Any?>): Map<String, Any?> {
        ...
        return if (hasAnalysisShape) cn.com.omnimind.bot.omniflow.function.OmniFlowFunctionJson.sanitizeMap(json) else emptyMap()
    }
    ...
    private fun directPatchShape(source: Map<String, Any?>): Map<String, Any?> {
        ...
        return cn.com.omnimind.bot.omniflow.function.OmniFlowFunctionJson.sanitizeMap(patch)
    }
    ...
    private fun mapFromAny(value: Any?): Map<String, Any?> =
        when (value) {
            is Map<*, *> -> cn.com.omnimind.bot.omniflow.function.OmniFlowFunctionJson.sanitizeMap(value)
    ...
    private companion object {
        const val TAG = "OmniFlowFunctionService"
        ...
        fun functionIdFromSpec(spec: Map<String, Any?>): String =
            OmniFlowFunctionSchema.functionIdFromSpec(spec)
        ...
        fun sourceRunIds(spec: Map<String, Any?>): List<String> =
            OmniFlowFunctionSchema.sourceRunIds(spec)
