...
object FunctionParameterBindingNormalizer {
    ...
    private fun actionBindingsForStep(
        ...
        val actions = FunctionJson.listArg(spec["actions"])
        val action = FunctionJson.mapArg(actions.getOrNull(stepIndex))
        if (action.isEmpty()) return emptyList()
        val actionType = FunctionJson.firstNonBlank(action["tool"])
