...
import cn.com.omnimind.bot.function.FunctionJson.boolArg
...
import cn.com.omnimind.bot.function.FunctionJson.mutableJsonMap
import cn.com.omnimind.bot.function.FunctionJson.sanitizeMap
import cn.com.omnimind.bot.function.FunctionJson.sanitizeValue
...
object FunctionSchema {
    ...
    private val capturedNoiseTools: Set<String> = setOf(
        "notification_send",
    ...
    fun isFunctionExecutor(raw: Any?): Boolean =
        raw?.toString()?.trim()?.lowercase() in setOf(EXECUTOR_FUNCTION, LEGACY_EXECUTOR_FUNCTION)
    ...
    fun callableSummary(spec: Map<String, Any?>): Map<String, Any?> {
        ...
        val parameters = inputSchema(spec)
    ...
    fun stepSummaries(spec: Map<String, Any?>): List<Map<String, Any?>> =
        materializedSteps(spec).mapIndexed { index, step ->
            ...
            linkedMapOf("index" to index, "id" to firstNonBlank(step["id"], "step_${index + 1}"),
                ...
                "kind" to step["kind"], "tool" to tool)
        }
    ...
    private fun canonicalActionToStep(index: Int, action: Map<String, Any?>): Map<String, Any?>? {
        ...
        return when {
            ...
            FunctionSchema.isFunctionCallTool(normalizedType) ->
            ...
            normalizedType == FunctionSchema.TOOL_EXTERNAL_TOOL ->
    ...
    private fun localActionStep(stepId: String, index: Int, title: String, action: String, args: Map<String, Any?>, sourceContext: Map<String, Any?>): Map<String, Any?> =
        linkedMapOf<String, Any?>("id" to stepId, "index" to index, "title" to title, "kind" to "function",
            "model_free" to true, "scriptable" to true,
    ...
    private fun functionStep(stepId: String, index: Int, title: String, args: Map<String, Any?>): Map<String, Any?> =
        linkedMapOf<String, Any?>("id" to stepId, "index" to index, "title" to title, "kind" to "function",
            "model_free" to true, "scriptable" to true,
            "tool" to OobActionSchema.TOOL_CALL_TOOL, "args" to args.filterValues { it != null })
    ...
    private fun externalToolStep(stepId: String, index: Int, title: String, toolName: String, args: Map<String, Any?>): Map<String, Any?> =
        linkedMapOf<String, Any?>("id" to stepId, "index" to index, "title" to title, "kind" to "tool_call",
            "scriptable" to true, "tool" to toolName, "args" to args)
