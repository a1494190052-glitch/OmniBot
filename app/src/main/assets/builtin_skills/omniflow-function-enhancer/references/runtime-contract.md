...
- Model-backed enhancement must be an explicit offline/background job with `auto_analyze_with_model=true`; online calls should only collect analysis context.
...
- A checker is a conditional injected action. It does not retarget or replace the main-path action; it runs in the single replay checkpoint between observe and action transfer when the current XML proves the condition.
