...
### Display 表达不限，交互必须一一对应

HTML Display 不限制表达能力：可以做复杂布局、图表、多视图、筛选、编辑面板和交互状态。限制的是后端交互协议：每一个会触发后端的前端动作，都必须一一对应到一个注册过的 Project API。

两条输入路由可以并存：

```
前端动作 -> data-oob-api="<toolId>" -> window.oob.callApi(toolId, inputs) -> Project API -> item.fields/project.items -> HTML 渲染
用户自然语言 -> Agent -> workbench_api_call(projectId, toolId, inputs) -> 同一个 Project API -> item.fields/project.items -> HTML 渲染
```

当前默认 Display renderer 是 `frontend/html/` 下的静态 HTML/CSS/JS。它默认负责：

- 丰富呈现 Project 数据和交互状态。
- 表单、按钮、筛选、图表、编辑面板等前端交互。
- `project.items` 列表/仪表展示。
- 归档、删除、轻量编辑等已存在数据操作。

HTML 可以收集输入和组织交互，但业务能力必须落在 Project Tool、Agent action、script 或 Project 文档里。不要在 HTML 里实现另一套数据模型、绕过 Project API、或直接改持久化文件。
...
- 可见交互：触发后端的按钮/控件必须标 `data-oob-api="<toolId>"`，并调用同一个 `toolId`。
- 输入映射：`callApi` 的 `inputs` key 必须来自对应 API 的 `inputSchema.properties`。
...
- 每个触发后端的可见控件都有 `data-oob-api="<toolId>"`，并与实际 `callApi(toolId)` 一致。
- `callApi` inputs 与对应 `inputSchema.properties` 一致；渲染字段与 `item.fields.*` 一致。
