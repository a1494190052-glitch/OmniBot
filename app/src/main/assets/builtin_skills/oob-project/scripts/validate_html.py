...
Focus: every frontend interaction that talks to the backend maps one-to-one
to a registered Project API.
...
def validate(html: str, registered_apis: list) -> list:
    ...
    # ── frontend/API one-to-one binding checks ────────────────────────────────
    called_ids = set(re.findall(r"""callApi\s*\(\s*['"]([^'"]+)['"]""", html))
    annotated_ids = set(re.findall(
        r"""data-oob-api\s*=\s*['"]([^'"]+)['"]""",
        html,
        flags=re.IGNORECASE,
    ))
    ...
    for api_id in sorted(annotated_ids):
        if api_id not in registered_api_ids:
            errors.append(
                f"FAIL [api] data-oob-api='{api_id}' not in registered apis — "
                "frontend/backend mapping is broken"
            )
    for api_id in sorted(called_ids - annotated_ids):
        errors.append(
            f"WARN [api] callApi('{api_id}') has no matching data-oob-api marker — "
            "mark the visible trigger so UI action and backend API stay one-to-one"
        )
    for api_id in sorted(annotated_ids - called_ids):
        errors.append(
            f"WARN [api] data-oob-api='{api_id}' is not called by HTML — "
            "remove the marker or wire it to window.oob.callApi"
        )
