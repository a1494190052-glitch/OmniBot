...
def extract_data_oob_api_ids(html):
...
def run(project_path):
    ...
    annotated_ids = extract_data_oob_api_ids(all_html)
    ...
    unregistered_markers = annotated_ids - registered_tool_ids
    if unregistered_markers:
        record(
            "FAIL",
            "all data-oob-api ids are registered",
            f"unregistered: {sorted(unregistered_markers)}",
        )
    elif annotated_ids:
        record("PASS", "all data-oob-api ids are registered")

    missing_markers = called_ids - annotated_ids
    if missing_markers:
        record(
            "WARN",
            "frontend callApi ids have visible data-oob-api markers",
            f"missing markers: {sorted(missing_markers)}",
        )
    elif called_ids:
        record("PASS", "frontend callApi ids have visible data-oob-api markers")

    unused_markers = annotated_ids - called_ids
    if unused_markers:
        record(
            "WARN",
            "data-oob-api markers are wired to callApi",
            f"not called in HTML: {sorted(unused_markers)}",
        )
    elif annotated_ids:
        record("PASS", "data-oob-api markers are wired to callApi")
