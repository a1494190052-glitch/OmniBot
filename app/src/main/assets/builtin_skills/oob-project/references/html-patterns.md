...
HTML Display can be expressive: complex layout, charts, filters, editors, multi-step flows,
and visual states are all allowed. The hard contract is backend interaction:
...
visible control data-oob-api="<toolId>"
  ...
  -> project.items / item.fields render
```

Controls that only open panels, switch tabs, filter local data, or change visual state do not
need `data-oob-api`. Any control that creates, edits, archives, analyzes, imports, exports,
or otherwise touches backend data/capabilities must carry `data-oob-api` and call the same id.
  ...
  el.innerHTML = items.map(item => `
    <div class="card" data-oob-id="card-${item.id}">
      <div class="card-row">
        ...
        <button class="btn-archive" data-oob-api="item.archive" onclick="archive('${item.id}')">归档</button>
      </div>
    </div>`).join('');
}
...
// apiId must match both a visible data-oob-api marker and an apis[].toolId entry.
// inputs keys must match that API's inputSchema.properties.
...
```html
<button id="btn-analyze" data-oob-api="meal.analyze" onclick="triggerAgentTask()">分析</button>
```
...
```html
<button data-oob-api="entry.stats" onclick="loadStats()">统计</button>
```
