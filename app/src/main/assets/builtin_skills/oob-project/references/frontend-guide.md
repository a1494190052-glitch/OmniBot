...
Use this guide when implementing or changing the HTML Display surface. HTML can be expressive: layouts, charts, filters, editors, multi-step panels, and rich interaction states are allowed. The hard rule is not visual simplicity; the hard rule is backend mapping. Every frontend interaction that touches data or capabilities must map one-to-one to a registered Project API.

Required mapping:

```
visible control data-oob-api="<toolId>"
  -> window.oob.callApi("<toolId>", inputs)
  -> apis[].toolId == "<toolId>"
  -> inputs keys match apis[].inputSchema.properties
  -> outputs/project.items render through item.fields.*
```

Natural-language actions use the same backend:

```
user asks Xiaowan
  -> Agent calls workbench_api_call(projectId, toolId, inputs)
  -> same Project API
  -> same persisted item.fields/project.items
  -> HTML updates via onProjectUpdated
```
...
<div class="container">
  ...
  <div id="form-panel" class="card" style="display:none" data-oob-id="form-panel">
    ...
    <div style="display:flex;gap:8px;margin-top:12px">
      ...
      <button class="btn-primary" data-oob-api="item.create" onclick="submitForm()" style="flex:2;margin:0">保存</button>
    </div>
  </div>
  ...
  el.innerHTML = items.map(item => `
    <div class="card" data-oob-id="item-${item.id}">
      <div class="card-row">
        ...
        <button class="btn-archive" data-oob-api="item.archive" onclick="archiveItem('${item.id}')">归档</button>
      </div>
...
### One-to-one interaction contract

Every backend-touching control must declare the API it calls:

```html
<button data-oob-api="entry.create" onclick="createEntry(this)">保存</button>
```

The handler must call the same API id:

```js
async function createEntry(btn) {
  const inputs = {
    title: document.getElementById('title').value.trim(),
    amount: Number(document.getElementById('amount').value || 0),
  };
  const result = await window.oob.callApi('entry.create', inputs);
  if (!result.success) { showError(result.errorMessage || '操作失败'); return; }
  render(activeViewItems(result.project));
}
```

`inputs` must use keys from `entry.create.inputSchema.properties`. Display rendering must read the persisted values from `item.fields.title`, `item.fields.amount`, etc. through `toViewItem()`.
