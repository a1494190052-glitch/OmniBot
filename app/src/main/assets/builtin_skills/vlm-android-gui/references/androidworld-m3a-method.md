...
- Store concrete action cards with `started_at_ms`, `finished_at_ms`,
  ...
  usage.
- Convert replayable cards into an OOB OmniFlow reusable Function.
...
Recall is not a flat reusable Function-store search:

1. Match the current page to a UDEG node.
2. Read that node's skill-like decision context.
3. Consider reusable Functions attached to that node.
4. Rank attached reusable Functions.
...
2. `replay`: convert the successful OOB RunLog to a reusable Function, reset the
   same task params, then replay the Function without a model call.
...
- Keep failure summaries and after-action evidence available to the next VLM
  turn.
- Do not replace Kotlin online VLM, RunLog collection, reusable Function registration,
