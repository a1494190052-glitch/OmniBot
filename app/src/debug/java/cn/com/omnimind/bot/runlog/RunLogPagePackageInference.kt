...
object RunLogPagePackageInference {
    ...
    fun effectivePackage(recordedPackage: String, xml: String, activityName: String?): String {
        ...
        if (inferred.isEmpty()) {
            return if (recorded.isEmpty() || recorded == "android") activityPackage else recorded
        }
    ...
    private fun dominantPackage(xml: String): String {
        ...
        return packageCounts(xml).maxWithOrNull(
    ...
    private fun isPackageLike(value: String): Boolean =
        value != "android" && value.contains('.') && PACKAGE_NAME.matches(value)

    private fun packagePrefixBeforeClassName(value: String): String {
        val segments = value.split('.')
        val firstClassSegment = segments.indexOfFirst { it.firstOrNull()?.isUpperCase() == true }
        return if (firstClassSegment <= 0) value else segments.take(firstClassSegment).joinToString(".")
    }

    private val PACKAGE_ATTR = Regex("""\bpackage\s*=\s*["']([^"']+)["']""")
    private val RESOURCE_ID_ATTR =
        Regex("""\bresource-id\s*=\s*["']([A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z0-9_]+)+):id/[^"']*["']""")
    private val COMPONENT_PREFIX =
        Regex("""\b([A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z0-9_]+)+)/""")
