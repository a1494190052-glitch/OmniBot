...
class OobOmniFlowLoopAcceptanceTest {
    ...
    fun `run function repairs current failed step online and resumes offline replay`() = runBlocking {
        ...
        try {
            ...
            assertEquals("offline_online_offline", summary["execution_mode"])
            assertEquals(1, (summary["offline_steps"] as Number).toInt())
            assertEquals(1, (summary["online_fallback_steps"] as Number).toInt())
            assertEquals(1, (summary["online_repair_steps"] as Number).toInt())
            assertEquals(1, (summary["online_repair_budget"] as Number).toInt())
            assertEquals(true, summary["online_repair_applied"])
            assertEquals(1, (summary["model_calls"] as Number).toInt())
            assertEquals(0L, (summary["total_tokens"] as Number).toLong())
