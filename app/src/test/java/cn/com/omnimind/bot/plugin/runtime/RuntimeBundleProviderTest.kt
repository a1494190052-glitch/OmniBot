...
class RuntimeBundleProviderTest {
    @Test
    fun `provider delegates lifecycle to generic runtime adapter`() = runBlocking {
        val plugin = object : OmniPlugin {}
        val adapter = RecordingAdapter(plugin)
        val provider = RuntimeBundleProvider(
        ...
        assertEquals(
            listOf("prepare:INSTALL", "prepare:UPDATE", "open", "remove"),
            adapter.calls,
        )
    }
