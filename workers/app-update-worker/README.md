...
- `GET /updates?currentVersion=0.5.0.3&edition=standard&source=worker&includeBeta=true`
  ...
  - Always returns `cloudServicePolicy`. When `MIN_CLOUD_SERVICE_VERSION` is
    configured, enforcing clients below that version block account operations
...
Set `MIN_CLOUD_SERVICE_VERSION` to the oldest version that may register, sign
in, or use official cloud AI. Comparison is numeric by dot-separated segment,
and the minimum is inclusive (`0.5.7` allows `0.5.7` and newer). Leave it empty
to disable the gate. `CLOUD_SERVICE_UPDATE_MESSAGE` optionally overrides the
message shown by blocked clients.
...
CLOUD_SERVICE_UPDATE_MESSAGE = "请升级到最新版后继续使用账号与官方 AI 服务"
```
