...
- `GET /updates?currentVersion=0.5.0.3&edition=standard&channel=vlm-core&source=worker&includeBeta=true`
  - Public endpoint used by the Android app.
  - `channel` isolates release streams. Missing or invalid values use `public`, preserving older clients.
...
- `PATCH /admin/releases/:tag`
  - Partial update (any of `version`, `track`, `channel`, `draft`, `prerelease`, `publishedAt`, `releaseUrl`, `releaseNotes`, `assets`).
...
| `blob11`/`blob12`/`blob13` | track / source / channel | — |
...
`OFFICIAL_VLM_OPERATION_ENABLED` is omitted or false, the service stays
disabled even when API base, API key, and model are all configured. Set it to
`true` explicitly to enable the hosted service.
...
## VLM-core Standard channel

Builds intended only for the `vlm-core` test group must embed and publish the
same isolated channel. This uploads only the Standard APK to Worker/R2 and does
...
The first channel-enabled APK must be installed once over the existing test
build. After that, only APKs built with `OMNIBOT_UPDATE_CHANNEL=vlm-core` query
this stream; ordinary `public` builds cannot see its releases.
...
5. publishes it only when repository variable `VLM_CORE_AUTO_PUBLISH_ENABLED` is `true`.

The publish variable intentionally defaults to disabled. Run the manual
`Deploy App Update Worker` workflow after configuring repository secrets
`CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID` and variable
`APP_UPDATE_R2_BUCKET`. A successful deployment enables future `vlm-core`
publishing. Publishing never installs an APK automatically: Android clients
still require the user to press `立即更新` and confirm the system installer.

Before committing runtime changes locally, synchronize the canonical sources:

```bash
python3 scripts/sync-embedded-omniflow-runtime.py
