...
例如，用户说“如果另一台手机有通知，就告诉那台设备上的小万我已经知道了”时，先用 `omnilink_devices` 找到目标，再用 `omnilink_subscribe_events` 订阅 `NOTIFICATION_UPSERTED` / `NOTIFICATION_REMOVED`。收到事件后，如果用户确实要求回信，再用 `omnilink_send_message` 发送；不要把这个流程固化成一个名为“通知确认”的专用工具。对于只要求一次查询的意图，完成一次读取即可，不要擅自建立长期订阅。

## 对话触发

- “列出协作设备” → `omnilink_devices`
- “查另一台手机的电量/是否在线/是否充电/网络状态” → `omnilink_devices`，从目标设备的 `reachable` 和 `readiness.device` 自己组合答案
- “现在读取另一台手机的通知” → `omnilink_read_events`，明确传入目标 `device_id` 和 `event_types=[NOTIFICATION_UPSERTED, NOTIFICATION_REMOVED]`，需要短等候时使用 `wait_ms`
- “开始持续接收另一台手机的通知” → `omnilink_subscribe_events`，明确传入同一组 `event_types` 和 `mode=start`
- “停止接收另一台手机的通知” → `omnilink_subscribe_events`，传入目标 `device_id` 和 `mode=stop`
- “给另一台设备上的小万发信号/消息：……” → `omnilink_send_message`
- “读取另一台设备的小万消息事件” → `omnilink_read_events`，传入 `event_types=[AGENT_MESSAGE_RECEIVED]`
...
5. 发送消息时把用户原意放进 `message`，不要把 token、私有路径或其他敏感数据放进消息。
6. 只在确实需要时指定 `conversation_id`、`recipient_agent_id`、`message_id`；省略时由插件使用小万 Agent 的安全默认值并生成幂等 ID。
7. `omnilink_read_events` 和 `omnilink_subscribe_events` 的 `event_types` 只能使用 Agent-safe 类型；不要猜测未注册事件类型。
8. 返回的 `cursor` 是不透明值，只能原样带回下一次读取；订阅由插件负责持久化游标和断线恢复。
9. `omnilink_send_message` 返回 `queued` 只代表已进入 OmniLink 队列，不代表对方小万已经读到；只有看到入站事件或明确 receipt 才能说“已收到”。
