...
internal class AccessibilityAndroidGuiPlatform(
    ...
    private suspend fun inputText(action: Action): AndroidGuiActionResult = withNodes { nodes ->
        val node = selectInputNode(
            nodes = nodes,
            x = optionalNumber(action, OobActionSchema.ARG_X),
            y = optionalNumber(action, OobActionSchema.ARG_Y),
            resourceId = action.args[OobActionSchema.ARG_NODE_RESOURCE_ID]?.toString().orEmpty(),
        ) ?: return@withNodes AndroidGuiActionResult(false, "input_target_not_found")
        if (node.isPassword) return@withNodes AndroidGuiActionResult(false, "password_input_not_supported")
        val text = action.args[OobActionSchema.ARG_TEXT]?.toString()
            ?: return@withNodes AndroidGuiActionResult(false, "input_text_required")
        val arguments = Bundle().apply {
            putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE, text)
        }
        val success = node.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT, arguments)
        AndroidGuiActionResult(success, if (success) "input_text_dispatched" else "input_text_failed")
    }
    ...
    private suspend fun pressKey(
        ...
        val success = when (key) {
            ...
            "enter" -> withNodes { nodes ->
                val node = selectInputNode(nodes, null, null) ?: return@withNodes false
