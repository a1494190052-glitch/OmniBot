  ...
  static const tools = <OobActionToolSpec>[
    OobActionToolSpec(
      ...
      args: <OobActionArgSpec>[
        ...
        OobActionArgSpec(name: "x", type: OobActionArgType.number, required: true, description: OobLocalizedText(zhCn: "兜底点击位置的 0..1000 相对 X 坐标；执行前会解码为屏幕绝对像素。", enUs: "Fallback 0..1000 relative X coordinate of the tap target; decoded to absolute screen pixels before execution."), minimum: 0),
        OobActionArgSpec(name: "y", type: OobActionArgType.number, required: true, description: OobLocalizedText(zhCn: "兜底点击位置的 0..1000 相对 Y 坐标；执行前会解码为屏幕绝对像素。", enUs: "Fallback 0..1000 relative Y coordinate of the tap target; decoded to absolute screen pixels before execution."), minimum: 0),
      ],
      modelVisible: true,
      replayable: true,
      editorVisible: true,
    ...
    OobActionToolSpec(
      ...
      args: <OobActionArgSpec>[
        ...
        OobActionArgSpec(name: "node_id", type: OobActionArgType.string, description: OobLocalizedText(zhCn: "可选。live XML 中已确认的无障碍节点 id。", enUs: "Optional. Confirmed accessibility node id from live XML.")),
        OobActionArgSpec(name: "node_resource_id", type: OobActionArgType.string, description: OobLocalizedText(zhCn: "可选。录制或 XML grounding 得到的 Android resource-id，用于重放定位诊断和迁移。", enUs: "Optional. Android resource-id captured from recording or XML grounding, used for replay targeting diagnostics and transfer.")),
        OobActionArgSpec(name: "x", type: OobActionArgType.number, required: true, description: OobLocalizedText(zhCn: "兜底目标输入框中心的 0..1000 相对 X 坐标；执行前会解码为屏幕绝对像素。", enUs: "Fallback 0..1000 relative X coordinate of the input target center; decoded to absolute screen pixels before execution."), minimum: 0),
        OobActionArgSpec(name: "y", type: OobActionArgType.number, required: true, description: OobLocalizedText(zhCn: "兜底目标输入框中心的 0..1000 相对 Y 坐标；执行前会解码为屏幕绝对像素。", enUs: "Fallback 0..1000 relative Y coordinate of the input target center; decoded to absolute screen pixels before execution."), minimum: 0),
      ],
      modelVisible: true,
      replayable: true,
      editorVisible: true,
