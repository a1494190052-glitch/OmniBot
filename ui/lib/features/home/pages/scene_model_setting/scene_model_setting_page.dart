...
class _SceneModelSettingPageState extends State<SceneModelSettingPage> {
  ...
  Future<void> _saveOperationConfig(SceneOperationConfig nextConfig) async {
    ...
    try {
      ...
      showToast(
        context.trLegacy(
          saved.useOfficialService ? '已启用 ChatGPT Luna' : '已切换为自定义 Provider',
        ),
        type: ToastType.success,
      );
    } catch (error) {
        ...
        Container(
          ...
          child: Row(
            children: [
              Expanded(
                child: Text(
                  context.trLegacy('默认使用 ChatGPT Luna'),
                  style: TextStyle(
                    color: _primaryTextColor,
                    fontSize: 12.5,
                    fontWeight: FontWeight.w500,
              ...
              KeyedSubtree(
                ...
                child: _buildCompactSettingsSwitch(
                  ...
                  semanticsLabel: context.trLegacy('使用 ChatGPT Luna'),
                  onToggle: (value) {
                    unawaited(
