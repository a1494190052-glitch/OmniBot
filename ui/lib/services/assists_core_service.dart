...
class AssistsMessageService {
  ...
  static Future<Map<String, dynamic>> updateOobFunction({
    ...
    if (!autoAnalyzeWithModel ||
        payload['needs_agent_analysis'] != true ||
        (analysis != null && analysis.isNotEmpty) ||
        (patch != null && patch.isNotEmpty)) {
      return payload;
    }
    return _completeOobFunctionUpdateWithAgentAnalysis(
  ...
  _completeOobFunctionUpdateWithAgentAnalysis({
    ...
    final prompt = (initialPayload['agent_prompt'] ?? '').toString().trim();
    if (prompt.isEmpty) {
      return {
        ...initialPayload,
        'success': false,
        'error_code': 'AGENT_PROMPT_EMPTY',
        'error_message':
            'update_function returned needs_agent_analysis without agent_prompt',
      };
    }
    var raw = await postLLMChat(
      text: prompt,
      model: 'scene.dispatch.model',
      responseJsonObject: true,
    );
    if ((raw ?? '').trim().isEmpty) {
      raw = await postLLMChat(
        text: prompt,
        model: 'scene.dispatch.model',
    ...
    final analysisPayload = _analysisFromAgentUpdateJson(aiJson);
    final patchPayload = _patchFromAgentUpdateJson(aiJson, analysisPayload);
    if (analysisPayload.isEmpty && patchPayload.isEmpty) {
      return {
        ...initialPayload,
        'success': false,
        'error_code': 'AGENT_ANALYSIS_EMPTY',
        'error_message':
            'Agent model returned no analysis or patch for update_function',
        'agent_model_invoked': true,
        'agent_response': aiJson,
      };
    }
    final nextExtraArgs = {
      ..._jsonSafeMap(extraArgs),
      'source': 'assists_core_service_agent_analysis',
      'agent_model_invoked': true,
    };
    final updated = await updateOobFunction(
      functionId: functionId,
      runId: runId,
      mode: mode,
      analysis: analysisPayload.isEmpty ? null : analysisPayload,
      patch: patchPayload.isEmpty ? null : patchPayload,
      extraArgs: nextExtraArgs,
      autoAnalyzeWithModel: false,
    );
    return {
      ...updated,
      'agent_model_invoked': true,
      'agent_analysis_initial': initialPayload,
    };
  }
  ...
  static Map<String, dynamic>? _extractJsonObject(String raw) {
    ...
    for (final candidate in <String>[
      ...
      try {
        ...
        final map = _jsonSafeDynamicMap(decoded);
        if (map.isNotEmpty) {
          return map;
        }
      } catch (_) {
  ...
  static String _substringJsonObject(String raw) {
  ...
  static Map<String, dynamic> _analysisFromAgentUpdateJson(
    Map<String, dynamic> json,
  ) {
    final direct = _firstNonEmptyMap(json, const [
      'analysis',
      'evidence_analysis',
      'runlog_analysis',
    ]);
    ...
    final toolArgs = _firstNonEmptyMap(json, const [
      'arguments',
      'args',
      'tool_args',
      'toolArgs',
    ...
    if (toolArgs.isNotEmpty) {
      final nested = _analysisFromAgentUpdateJson(toolArgs);
      if (nested.isNotEmpty) {
        return nested;
      }
    }
    final hasAnalysisShape = const [
      'summary',
      'failure_reason',
      'recommended_patch',
      'evidence',
  ...
  static Map<String, dynamic> _patchFromAgentUpdateJson(
    Map<String, dynamic> json,
    Map<String, dynamic> analysis,
  ) {
    final direct = _firstNonEmptyMap(json, const [
    ...
    final analysisPatch = _firstNonEmptyMap(analysis, const [
      'recommended_patch',
      'patch',
      'function_patch',
    ]);
    if (analysisPatch.isNotEmpty) {
    ...
    if (toolArgs.isNotEmpty) {
      return _patchFromAgentUpdateJson(toolArgs, analysis);
    }
    return <String, dynamic>{};
  }
  ...
  static Map<String, dynamic> _firstNonEmptyMap(
    ...
    for (final key in keys) {
      final map = _jsonSafeDynamicMap(source[key]);
      if (map.isNotEmpty) {
        return map;
      }
    }
