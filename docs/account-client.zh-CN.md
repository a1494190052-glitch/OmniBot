...
主聊天文本、图片生成和语音播放都读取这个模式：平台模式携带账号 Access Token 访问品牌网关，并只使用网关模型目录声明的能力；BYOK 模式继续使用设备上的模型提供商、图片生成与语音配置。切换到平台模式时，本地 BYOK Provider、Key 和主文本选择会被保留；切回 BYOK 或退出账号时会恢复原来的本地选择。升级前已经配置的 BYOK Provider 会继续有效，新建或修改第三方地址时仍需要确认数据接收方。若 BYOK 图片 Provider 没有 Key，而构建时配置了 `OMNIBOT_IMAGE_API_KEY`，图片工具会继续使用小万内置图片服务作为回退。
