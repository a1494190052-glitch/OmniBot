...
def parse_function_artifact(value: dict[str, Any]) -> Function:
    ...
    for index, step in enumerate(raw_steps):
        if not isinstance(step, dict) or set(step) - {"step_index", "source_state_id", "action"}:
...
def validate_function_artifact(function: Function) -> None:
    ...
    for step in function.steps:
        if canonicalize_action(
            step.action.to_dict(), replayable_only=True
        ) != step.action.to_dict():
...
def bind_function(function: Function, arguments: dict[str, Any]) -> Function:
    ...
    for binding in function.bindings:
        ...
        value = _read_path(source_root, _tokens(".arguments" + source_match.group("tail")))
...
def _validate_bindings(function: Function) -> None:
    ...
    unbound = sorted(set(function.input_schema.get("required") or ()) - bound_properties)
    if unbound:
...
def _tokens(tail: str) -> list[str | int]:
    return [
        ...
        for name, index in _PATH_TOKEN.findall(tail)
    ]
