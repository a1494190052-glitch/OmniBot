...
from omniflow.gui import build_model_turn_request, parse_model_turn_response
from omniflow.model import Observation
from omniflow.schemas import load_canonical_action_schema
...
def test_model_request_uses_canonical_tools_and_one_native_call() -> None:
    ...
    click = next(tool for tool in request["tools"] if tool["function"]["name"] == "click")
    ...
    assert "target_description" in parameters["properties"]
    assert parameters["properties"]["x"]["maximum"] == 1000
    assert request["tool_choice"] == "required"
    assert request["parallel_tool_calls"] is False
    assert "wait" not in tool_names
    assert "Browser=browser" in request["messages"][1]["content"][0]["text"]
...
@pytest.mark.parametrize(
    "case",
    _VALID_TOOL_CALLS,
    ids=[case["id"] for case in _VALID_TOOL_CALLS],
)
def test_valid_tool_call_dataset_binds_every_visible_action(case: dict) -> None:
...
@pytest.mark.parametrize(
    "case",
    _INVALID_TOOL_CALLS,
    ids=[case["id"] for case in _INVALID_TOOL_CALLS],
)
...
@pytest.mark.parametrize(
    "case",
    _ADAPTED_TOOL_CALLS,
    ids=[case["id"] for case in _ADAPTED_TOOL_CALLS],
)
...
@pytest.mark.parametrize(
    "case",
    _ADAPTER_REJECTION_CASES,
    ids=[case["id"] for case in _ADAPTER_REJECTION_CASES],
)
...
def test_model_response_allows_coordinate_action_without_grounding_description() -> None:
    action, metadata = parse_model_turn_response(
        _response(
            "click",
...
def test_model_response_rejects_missing_or_multiple_native_calls() -> None:
    for tool_calls in ([], [_response("wait", {"summary": "等待", "duration_ms": 1})["tool_calls"][0]] * 2):
...
def test_bridge_planner_retries_rejected_tool_call_with_only_its_canonical_schema() -> None:
    class FakeBridge:
    ...
    assert "Coordinate fields such as x and y must each be one JSON number" in correction_text


def test_bridge_planner_accepts_qwen_adapter_without_retrying_or_forking_execution() -> None:
    class FakeBridge:
        def __init__(self) -> None:
            self.requests: list[dict] = []
...
@pytest.mark.parametrize(
    ...
    [
        ...
        if case["tool"] in {seed["tool"] for seed in _VALID_TOOL_CALLS}
    ],
    ids=[
        case["id"]
        for case in _INVALID_TOOL_CALLS
...
def test_rejected_argument_dataset_reselects_with_one_canonical_tool(
    ...
    valid_case = next(
        seed for seed in _VALID_TOOL_CALLS if seed["tool"] == case["tool"]
    )
...
def test_bridge_planner_stops_after_three_invalid_tool_calls() -> None:
    class FakeBridge:
        def __init__(self) -> None:
            self.requests: list[dict] = []
