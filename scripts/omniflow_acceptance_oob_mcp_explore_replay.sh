...
if replay:
    ...
    if replay_result.get("success") is not True:
        fail("replay_failed", "explore_replay did not replay the generated reusable Function successfully", result)
...
if guard.get("decision") not in {"allow", "needs_agent"}:
    fail("guard_rejected_generated_function", "Generated reusable Function guard did not allow replay", guard)
...
if replayable_steps < 1:
    fail("replayable_function_empty", "Generated reusable Function does not contain replayable steps", function_spec)
