...
KOTLIN_OUT = ROOT / "baselib/src/main/java/cn/com/omnimind/baselib/runlog/OobActionSchema.kt"
DART_OUT = ROOT / "ui/lib/features/task/run_log/oob_canonical_action_schema.dart"


def read_schema() -> dict[str, Any]:
    schema = json.loads(SCHEMA_PATH.read_text(encoding="utf-8"))
    allowed_kinds = {"action", "observe", "function_invocation", "decision"}
    for tool in schema.get("tools") or ():
        kind = tool.get("kind")
        if kind not in allowed_kinds:
            raise ValueError(f"Unsupported action kind for {tool.get('name')}: {kind}")
    return schema
...
def kt_arg(arg: dict[str, Any], indent: str = "                ") -> str:
    ...
    if not arg.get("persisted", True):
        parts.append("persisted = false")
...
def dart_kind(value: str) -> str:
    return {
        "action": "OobActionKind.action",
        "observe": "OobActionKind.observe",
        "function_invocation": "OobActionKind.functionInvocation",
        "decision": "OobActionKind.decision",
    }[value]
...
{arg_consts}
  ...
  static const actionToolNames = <String>{{{", ".join(dart_str(tool["name"]) for tool in tools if tool["kind"] == "action")}}};
...
def main() -> None:
    schema = read_schema()
    kotlin = generate_kotlin(schema)
    dart = generate_dart(schema)
    if "--check" in sys.argv[1:]:
        stale = [
            path.relative_to(ROOT)
            for path, expected in ((KOTLIN_OUT, kotlin), (DART_OUT, dart))
            if not path.is_file() or path.read_text(encoding="utf-8") != expected
        ]
        if stale:
            print(
                "Generated action schema files are stale: "
                + ", ".join(map(str, stale)),
                file=sys.stderr,
            )
            print(
            ...
            raise SystemExit(1)
        print("Generated action schema files are up to date")
        return
    KOTLIN_OUT.write_text(kotlin, encoding="utf-8")
    DART_OUT.write_text(dart, encoding="utf-8")
