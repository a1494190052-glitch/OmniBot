...
prompt = kit.agent_prompt("Run the safest saved reusable Function", report.summary())
...
assert "Run the safest saved reusable Function" in prompt
...
PY
  ...
  STEP_START_MS="$(now_ms)"
  "$VENV_DIR/bin/omniflow-agentkit" prompt "Run the safest saved reusable Function" --repo . >"$WORK_ROOT/omniflow-agentkit-prompt.txt"
